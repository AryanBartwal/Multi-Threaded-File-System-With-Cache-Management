...
public:
    static void simulate_operation(const std::string& operation_name,
        ...
        for (int i = 0; i < iterations; ++i) {
            ...
            std::cout << std::setw(9) << (i + 1) 
                     << " | " << std::setw(11) << std_duration.count() << "μs"
                     << " | " << std::setw(9) << custom_duration.count() << "μs"
    ...
    static void compare_with_stats(const std::string& name, 
        ...
        std::cout << "----------------|-------------|-------------|--------" << std::endl;
        std::cout << "Average         | " << std::setw(9) << std_avg << "μs | " 
                 << std::setw(9) << custom_avg << "μs | " 
                 << std::fixed << std::setprecision(2) << (double)custom_avg/std_avg << "x" << std::endl;
        std::cout << "Best (Min)      | " << std::setw(9) << std_min << "μs | " 
                 << std::setw(9) << custom_min << "μs | " 
                 << std::fixed << std::setprecision(2) << (double)custom_min/std_min << "x" << std::endl;
        std::cout << "Worst (Max)     | " << std::setw(9) << std_max << "μs | " 
                 << std::setw(9) << custom_max << "μs | " 
...
public:
    static void benchmark_directory_create(int num_dirs) {
        ...
        std::cout << "Standard filesystem: " << std_duration.count() << " μs" << std::endl;
        std::cout << "Custom filesystem:   " << custom_duration.count() << " μs" << std::endl;
    ...
    static void benchmark_directory_list(const std::string& path) {
        ...
        std::cout << "Standard filesystem: " << std_duration.count() << " μs (" << std_files.size() << " files)" << std::endl;
        std::cout << "Custom filesystem:   " << custom_duration.count() << " μs (" << custom_files.size() << " files)" << std::endl;
    }
};
...
public:
    ...
    static void benchmark_file_find(const std::string& search_pattern, const std::string& search_dir) {
        ...
        std::cout << "Standard filesystem: " << std_duration.count() << " μs (" << std_found.size() << " files found)" << std::endl;
        std::cout << "Custom filesystem:   " << custom_duration.count() << " μs (" << custom_found.size() << " files found)" << std::endl;
    }
...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        if (run_comprehensive) {
            ...
            std::cout << "File Write: " << write_duration.count() << " μs" << std::endl;
            std::cout << "File Read:  " << read_duration.count() << " μs" << std::endl;
            std::filesystem::remove(filename);
            ...
            for (int i = 0; i < operations; ++i) {
                if (i % 2 == 0) {
                    std_cache[i % 1000] = "value_" + std::to_string(i);
                } else {
                    auto it = std_cache.find(i % 1000);
                    if (it != std_cache.end()) {
            ...
            for (int i = 0; i < operations; ++i) {
                if (i % 2 == 0) {
                    custom_cache.put(i % 1000, "value_" + std::to_string(i));
                } else {
                    std::string value = custom_cache.get(i % 1000);
                }
            }
            end = std::chrono::high_resolution_clock::now();
            auto custom_duration = std::chrono::duration_cast<std::chrono::microseconds>(end - start);
            ...
            std::cout << "Custom LRU cache:       " << custom_duration.count() << " μs" << std::endl;
