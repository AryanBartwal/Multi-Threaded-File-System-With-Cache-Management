...
void run_cache_benchmarks() {
    ...
    // YOUR CUSTOM: mtfs::cache::LRUCache (with eviction, limited size)
    SimpleBenchmark::benchmark("YOUR CUSTOM: mtfs::cache::LRUCache", [&]() {
        mtfs::cache::LRUCache<std::string, std::string> your_cache(100); // Limited to 100 items
    ...
    SimpleBenchmark::benchmark("REALISTIC: LRU Cache Hot Data (90% hits)", [&]() {
        mtfs::cache::LRUCache<std::string, std::string> hot_cache(50);
...
void run_filesystem_benchmarks() {
    ...
    SimpleBenchmark::benchmark("YOUR CUSTOM: mtfs::fs::FileSystem", [&]() {
        try {
            mtfs::fs::FileSystem your_fs("test_fs_root");
            
            // Write (with your custom caching and compression)
...
void run_backup_benchmarks() {
    ...
    SimpleBenchmark::benchmark("YOUR CUSTOM: mtfs::fs::BackupManager", [&]() {
        try {
            mtfs::fs::BackupManager your_backup;
            
            // Create backup with your custom features (compression, metadata, etc.)
            your_backup.createBackup("test_source", "your_backup_dest");
            
            std::filesystem::remove_all("your_backup_dest");
        } catch (...) {
            // Handle any errors
        }
    }, 20);
...
void run_compression_benchmarks() {
    ...
    // YOUR CUSTOM: mtfs::fs::Compression
    SimpleBenchmark::benchmark("YOUR CUSTOM: mtfs::fs::Compression", [&]() {
        try {
            mtfs::fs::Compression your_compressor;
            
            // Compress
            auto compressed = your_compressor.compress(repetitive_data);
            
            // Decompress to verify
            auto decompressed = your_compressor.decompress(compressed);
    ...
    try {
        mtfs::fs::Compression compressor;
        auto compressed = compressor.compress(repetitive_data);
...
int main(int argc, char* argv[]) {
    ...
    std::cout << "• YOUR LRUCache: Provides memory-bounded caching with LRU eviction" << std::endl;
    ...
    std::cout << "• YOUR Compression: Reduces storage space and I/O time" << std::endl;
